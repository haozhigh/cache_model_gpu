...
void update_stack_tree(unsigned long line_addr,
        unsigned set,
...
void reuse_distance(std::vector<unsigned> &core,
                    ...
                    map_type<unsigned,map_type<unsigned,unsigned>> &distances,
    ...
    */
    ...
    for (unsigned i = 0; i < core.size(); i++) {
        unsigned bid = core[i];
        for (unsigned j = 0; j < blocks[bid].size(); j++) {
            unsigned wid = blocks[bid][j];
            for (unsigned k = 0; k < warps[wid].size(); k++) {
                unsigned tid = warps[wid][k];
                while(!threads[tid].is_done()) {
                    //std::cout<<i<<'/'<<core.size()<<"  "<<j<<'/'<<blocks[bid].size()<<"  "<<k<<'/'<<warps[wid].size()<<"  "<<threads[tid].pc<<'/'<<threads[tid].accesses.size()<<std::endl;
                    Access access = threads[tid].schedule();
    ...
    */
    ...
    //list to record all the ongoing requests 
    std::list<unsigned> ongoing_requests;
		...
		while (!pool.is_done()) {
            ...
            */
			...
			if (pool.has_work()) {
                ...
                //MSHR check
                int requests_this_warp = 0;
                for (int i = 0; i < warps[wnum].size(); i++) {
                    if (threads[warps[wnum][i]].is_done())
                        continue;
                    Access tmp_access;
                    tmp_access = threads[warps[wnum][i]].schedule();
                    if (tmp_access.width != 0)
                        requests_this_warp++;
                    threads[warps[wnum][i]].unschedule();
                }
                do {
                    std::list<unsigned>::iterator it = ongoing_requests.begin();
                    ...
                    if (requests_this_warp + ongoing_requests.size() > num_mshr) {
                ...
                } while (requests_this_warp + ongoing_requests.size() > num_mshr);
				...
				for (unsigned warp_portion = 0; warp_portion < portions; warp_portion++) {
					...
					for (unsigned tnum = tnum_start; tnum < tnum_stop && tnum < warps[wnum].size(); tnum++) {
						...
						if (threads[tid].is_done()) {
							...
							Access access = threads[tid].schedule();
                            //Modified check thread done again, it's a bug
                            if (threads[tid].is_done()) {
                                threads_done++;
                            }
							if (access.width != 0) {
							...
							// Find the reuse distance
								...
								if (distance >= cache_ways) {
                                    ...
                                    ongoing_requests.push_back(memory_latency);
                                    // Consider the block or unblock situation
                                    if (access.block == 0)
                                        memory_latency = non_mem_latency;
                                    ...
                                    */
                                    ...
                                    update_stack_tree(line_addr, set, P, B, set_counters);
								}
								...
								if (!(distances[access.pc][distance])) {
									distances[access.pc][distance] = 0;
								}
								distances[access.pc][distance]++;
							}
						}
					}
