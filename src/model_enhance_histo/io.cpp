...
//////////////////////////////////
std::string trace_dir = "../output/trace_enhance";
std::string output_dir = "../output/model_enhance_histo";
...
//////////////////////////////////
void output_miss_rate(std::vector<map_type<unsigned,map_type<unsigned,unsigned>>> &distances,
                      const std::string kernelname,
                      const std::string benchname,
                      const std::string suitename,
	...
	std::ofstream file;
	file.open(output_dir+"/"+suitename+"/"+benchname+"/"+kernelname+".out");
	...
	std::vector<map_type<unsigned,unsigned>> total_distances(NUM_CASES);

    file << std::endl << "histogram:" << std::endl;
    for(auto it1 = distances[0].begin(); it1 != distances[0].end(); it1++) {
        for (auto it2 = it1->second.begin(); it2 != it1->second.end(); it2++) {
            file << "" << it1->first << " " << it2->first << " " << it2->second << std::endl;
            if (!total_distances[0][it2->first]) {
                total_distances[0][it2->first] = 0;
            }
            total_distances[0][it2->first] += it2->second;
        }
    }
    file << std::endl;
	...
	unsigned miss_compulsory[NUM_CASES] = {0};
	unsigned miss_capacity[NUM_CASES] = {0};
	unsigned miss[NUM_CASES];
	unsigned hits = 0;
	
	// Compute the cache misses for the 4 different cases
	for (int i=0; i<NUM_CASES; i++) {
		for(map_type<unsigned,unsigned>::iterator it=total_distances[i].begin(); it!= total_distances[i].end(); it++) {
        ...
        file << "CASE: " << i << " miss_compulsory: " << miss_compulsory[i] << std::endl;
        file << "CASE: " << i << " miss_capacity: " << miss_capacity[i] << std::endl;
        file << "modelled_hits: " << hits << std::endl;
	}
	
    /*
      comment out the redundant output statements for modified model
